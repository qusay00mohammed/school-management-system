...
<div class="row">
  <div class="col-md-12 mb-30">
    <div class="card card-statistics h-100">
      <div class="card-body">
        @can('add questions')
        <a href="{{ route('question.create') }}" class="btn btn-success btn-sm" role="button" aria-pressed="true">{{ __('trans_student.add new question') }}</a>
        <br><br>
        @endcan
            ...
            <tbody>
              @foreach ($questions as $question)
                <tr>
                  ...
                  <td>
                    @can('edit questions')
                    <a href="{{ route('question.edit', $question->id) }}" class="btn btn-info btn-sm" role="button"
                        aria-pressed="true"><i class="fa fa-edit"></i></a>
                    @endcan
                    @can('delete questions')
                    <button type="button" class="btn btn-danger btn-sm" data-toggle="modal"
                    data-target="#delete_question{{ $question->id }}" title="{{ __('trans_student.delete') }}"><i class="fa fa-trash"></i></button>
                    @endcan
                  </td>
                </tr>
